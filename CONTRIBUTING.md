...
We love your input! We want to make contributing to gloo.foo as easy and transparent as possible, whether it's:
...
gloo.foo uses a **modular architecture** where **each command is its own independent Go module**. This design choice is fundamental to gloo.foo's extensibility:
...
**Important**: gloo.foo follows an intentional naming pattern where the **package name matches the primary function name**:
...
- Enables dot imports: `import . "github.com/yupsh/grep"` then `Grep("pattern")` - cleaner for scripts (commands remain in yupsh org)
...
import (
    ...
    localopt "github.com/yourorg/mycommand/opt"
)

// Command implementation using StandardCommand abstraction
type command struct {
    gloo.StandardCommand[localopt.Flags]
}
...
func Mycommand(parameters ...any) gloo.Line {
    ...
    return command{
        StandardCommand: gloo.StandardCommand[localopt.Flags]{
...
We use GitHub issues to track public bugs. Report a bug by [opening a new issue](https://github.com/gloo-foo/framework/issues).
...
- [Architecture Overview](../gloo-foo/README.md)
